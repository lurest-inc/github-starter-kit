...
| Repository に Issue Label を一括追加したい | [④ Issue Label 一括作成](workflows/04-setup-repository-labels.md) | 設定ファイルで定義した Issue Label を Repository に一括作成 |
| Repository に初期ファイルを一括作成したい | [⑤ 初期ファイル一括作成](workflows/05-setup-repository-files.md) | Community Health Files・Scaffold ファイルを空ファイルとして一括登録 |
| Repository に Branch Ruleset を一括作成したい | [⑥ Ruleset 一括作成](workflows/06-setup-repository-rulesets.md) | 設定ファイルで定義した Branch Ruleset を Repository に一括作成 |
| Issue/PR をまとめて取り込みたい | [⑦ Issue/PR 一括紐付け](workflows/07-add-items-to-project.md) | Project に Repository の Issue/PR を一括追加 |

### 📈 分析・レポートする

| やりたいこと | Workflow / スクリプト | 説明 |
|-------------|----------------------|------|
| Project の内容を一覧で出したい | [⑧ 統合 Project 分析](workflows/08-analyze-project.md) | Project の Issue/PR 一覧をエクスポート（`report_types: export`） |
| 滞留タスクを検知したい | [⑧ 統合 Project 分析](workflows/08-analyze-project.md) | 指定日数以上動きのないアイテムを自動検出（`report_types: stale`） |
| サマリーレポートを出したい | [⑧ 統合 Project 分析](workflows/08-analyze-project.md) | Status 別・担当者別の集計レポートを生成（`report_types: summary`） |
| 工数・ベロシティを把握したい | [⑧ 統合 Project 分析](workflows/08-analyze-project.md) | 工数集計・ベロシティレポートを生成（`report_types: effort,velocity`） |
