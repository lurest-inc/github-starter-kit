...
| Project の内容を一覧で出したい | [④ Project アイテム エクスポート](workflows/04-export-project-items) / [⑩ 統合プロジェクト分析](workflows/10-analyze-project) | `Project` の `Issue`/`PR` 一覧をエクスポート（⑩では他の分析と一括実行可能） |
| リポジトリにラベルを一括追加したい | [⑤ Issue ラベル一括追加](workflows/05-setup-repository-labels) | 設定ファイルで定義したラベルをリポジトリに一括作成 |
| プロジェクトを分析したい | [⑩ 統合プロジェクト分析](workflows/10-analyze-project) | 滞留検知・サマリー・工数集計・エクスポートをまとめて、または個別に実行 |
