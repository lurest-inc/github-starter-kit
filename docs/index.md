...
| ワークフロー | 説明 | トリガー |
|------------|------|---------|
| [① GitHub Project 新規作成](01-create-project.md) | Project の作成・フィールド・ステータス・View を一括セットアップ | `workflow_dispatch`（手動実行） |
| [② GitHub Project 拡張](02-extend-project.md) | 既存 Project にフィールド・ステータス・View を追加 | `workflow_dispatch`（手動実行） |
| [③ Issue/PR 一括紐付け](03-add-items-to-project.md) | リポジトリの Issue/PR を Project に一括追加 | `workflow_dispatch`（手動実行） |
