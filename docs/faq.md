...
A. ワークフロー ④ の実行完了後、Actions タブの該当ワークフロー実行ページにある **Artifacts** セクションからダウンロードできます。保持期間は 7 日間です。

---

## 💡 Q6. 同じ Issue/PR を複数回追加してしまいませんか？

A. ワークフロー ③ は既に Project に追加済みのアイテムを自動的にスキップします。同じワークフローを何度実行しても重複追加は発生しません。
