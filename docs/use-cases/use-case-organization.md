...
→ [Workflow ⑧ 統合 Project 分析](../workflows/08-analyze-project.md)
...
4. **Workflow ⑥** で Branch Protection の Ruleset を一括設定し、コード品質を担保
5. **Workflow ⑦** で既存の Issue/PR を Project に一括紐付け
6. チームメンバーは初日から統一された Project ボードで作業開始
...
2. **Workflow ⑤** で各 Repository に統一された初期ファイルを一括作成
3. **Workflow ⑥** で Ruleset を一括設定し、Organization 全体のブランチ保護ルールを標準化
4. 週次で **Workflow ⑧** を実行し、サマリーレポート・ベロシティレポートを自動生成
5. 滞留タスクが自動検知され、対応漏れを防止
