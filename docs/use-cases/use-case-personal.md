...
2. **Workflow ⑤** で README・CONTRIBUTING 等の初期ファイルを一括作成
3. **Workflow ⑥** で Branch Protection の Ruleset を一括設定
4. **Workflow ⑦** で既存の Issue/PR を Project に一括紐付け
5. Board View で優先度ごとにタスクを整理し、リリースまでの進捗を一覧化
6. リリース前に **Workflow ⑧** で滞留タスクを検知し、対応漏れを防止
