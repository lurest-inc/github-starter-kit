...
| 2 | Status別件数 | 各 Status の件数と割合（Mermaid 円グラフ付き） |
| 3 | 担当者別件数 | 各担当者の Item 数と In Progress / In Review 内訳 |
| 4 | Label別件数 | 各 Label の Item 数 |
...
| 6 | 期日超過Item | 終了期日を過ぎた未完了 Item の一覧 |
...
| セクション | 内容 |
|-----------|------|
| Status別 | Status 名、件数、割合（テーブル + Mermaid 円グラフ） |
...
| 4 | 乖離Item | 見積もりと実績の乖離が大きい Item 一覧（上位 10 件） |
| 5 | 工数未入力Item | 見積もり・実績ともに未入力の Item 一覧 |
...
| Status別工数 | Status 名、Item 数、見積もり・実績工数、消化率 |
...
この Workflow で生成されるレポートは `GitHub Actions` の Artifact としてアップロードされます。Artifactの公開範囲について、以下の点にご注意ください。

### Artifactのアクセス範囲

- Artifact は **Repositoryの `Actions` タブにアクセスできるユーザーであれば誰でもダウンロード可能** です
