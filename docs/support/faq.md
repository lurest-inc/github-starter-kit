...
A. Workflow ⑦（[Issue/PR 一括紐付け](../workflows/07-add-items-to-project.md)）は既に Project に追加済みの Item を自動的にスキップします。同じ Workflow を何度実行しても重複追加は発生しません。

---

## 💡 Q7. エクスポートしたファイルはどこからダウンロードできますか？

A. Workflow ⑧（[統合プロジェクト分析](../workflows/08-analyze-project.md)）の実行完了後、`Actions` タブの該当 Workflow 実行ページにある **Artifacts** セクションからダウンロードできます。保持期間は `retention_days` パラメータで指定した日数（デフォルト: 7 日）です。
